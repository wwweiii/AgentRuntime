...
class ContextStore:
    ...
    def _semantic_lookup_with_embedding(self, emb: list[float]) -> str | None:
        if not emb or not hasattr(self, '_cosine'):
            return None
        # Walk existing embeddings linearly.  For ≤ a few hundred segments
        # this is fine; swap in a FAISS index when scale demands it.
        best_id, best_score = None, 0.0
        for seg_id, existing in self._embeddings_list():
    ...
    def _embeddings_list(self):
        """Iterate over (seg_id, embedding) pairs."""
        if not hasattr(self, '_embeddings'):
            return
        yield from self._embeddings.items()
